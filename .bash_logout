...
# Session Information
# ===================
local session_start_time="${BASH_LOGIN_TIME:-unknown}"
...
cleanup_session() {
    ...
    if [[ -n "$BASH_SESSION_ID" ]]; then
        find /tmp -name "*$BASH_SESSION_ID*" -user "$USER" -delete 2>/dev/null || true
    fi

    # Clean up any temporary completion files
    find "${XDG_CACHE_HOME:-$HOME/.cache}" -name "bash_completion_*" -user "$USER" -mtime +7 -delete 2>/dev/null || true
    ...
    local log_file="${XDG_STATE_HOME:-$HOME/.local/state}/bash/sessions.log"
    [[ ! -d "$(dirname "$log_file")" ]] && mkdir -p "$(dirname "$log_file")"

    local command_count="${HISTCMD:-0}"
    echo "$(date '+%Y-%m-%d %H:%M:%S') - Session ended: Duration: $session_duration, Commands: $command_count" >> "$log_file" 2>/dev/null || true
...
manage_ssh_agent() {
    ...
    if [[ -n "$SSH_AGENT_PID" ]] && [[ "$SHLVL" -eq 1 ]]; then
        ...
        if kill -0 "$SSH_AGENT_PID" 2>/dev/null; then
            ...
            local agent_users
            agent_users=$(pgrep -f "ssh-agent" -u "$USER" | wc -l)
...
cleanup_dev_environment() {
    ...
    if [[ -n "$PWD" && "$PWD" != "$HOME" ]]; then
        echo "$PWD" > "${XDG_STATE_HOME:-$HOME/.local/state}/bash/last_dir" 2>/dev/null || true
    fi
    ...
    if command -v docker >/dev/null 2>&1 && [[ -n "$BASH_SESSION_ID" ]]; then
        docker ps -a --filter "label=bash_session=$BASH_SESSION_ID" -q 2>/dev/null | \
...
show_farewell_message() {
    ...
    if [[ -t 1 ]] && [[ -n "$PS1" ]]; then
        ...
        # Colors for output
        if [[ "$TERM" != "dumb" ]] && tput setaf 1 >/dev/null 2>&1; then
            local BLUE='\033[34m'
            local GREEN='\033[32m'
        ...
        center_text() {
            ...
            local color="${2:-$RESET}"
            local plain_text
            plain_text=$(echo "$text" | sed 's/\x1B\[[0-9;]*[JKmsu]//g')
        ...
        local random_farewell="${farewells[$random_index]}"
...
if [[ -n "$BASH_PROFILE_STARTUP" ]]; then
    echo "$(date '+%Y-%m-%d %H:%M:%S'): .bash_logout completed" >> "${XDG_STATE_HOME:-$HOME/.local/state}/bash/startup.log"
fi
...
unset BASH_SESSION_ID BASH_LOGIN_TIME 2>/dev/null || true
